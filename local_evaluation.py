...
class CRAGEvaluator:
    ...
    def attempt_api_call(
        ...
        """
        Attempt a structured output call to the Qwen2 model with retries.
        """
        prompt = "\n".join([f"{m['role'].capitalize()}: {m['content']}" for m in messages])
        for attempt in range(max_retries):
            try:
                result = self.qwen2_pipeline(prompt)[0]["generated_text"]
                # Try to extract JSON from the output
                import re, json
                match = re.search(r"\{.*\}", result, re.DOTALL)
                if match:
                    parsed = json.loads(match.group(0))
                    return CRAGTurnEvaluationResult(**parsed)
            except Exception as e:
                if attempt == max_retries - 1:
                    console.print(f"[red]Qwen2 call failed after {max_retries} attempts: {str(e)}[/red]")
                else:
                    console.print(f"[yellow]Qwen2 call failed, retrying... {str(e)}[/yellow]")
        return None
    ...
    def evaluate_response(self, crag_turn_data: dict[str, any]) -> dict[str, any]:
        ...
        if not is_idk and not is_exact_match and self.eval_model_name:
            if self.qwen2_pipeline:
                messages = [
                    {"role": "system", "content": self.get_system_message()},
                    {"role": "user", "content": f"Question: {query}\nGround truth: {ground_truth}\nPrediction: {agent_response}\n"},
                ]
                api_response = self.attempt_api_call(self.eval_model_name, messages)
                if api_response:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Qwen model for semantic evaluation. Pass 'None' to disable semantic evaluation.",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    search_api_text_model_name = "intfloat/e5-large-v2"
